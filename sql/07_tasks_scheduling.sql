...
SELECT
    ...
    TRY_CAST(PARSE_JSON(PAYLOAD):state_code::VARCHAR AS NUMBER),
    TRY_CAST(PARSE_JSON(PAYLOAD):status_code::VARCHAR AS NUMBER),
    TRY_CAST(PARSE_JSON(PAYLOAD):reason_code::VARCHAR AS NUMBER),
    CASE WHEN TRY_CAST(PARSE_JSON(PAYLOAD):state_code::VARCHAR AS NUMBER) = 800 THEN TRUE ELSE FALSE END,
    CASE
        ...
        WHEN TRY_CAST(PARSE_JSON(PAYLOAD):state_code::VARCHAR AS NUMBER) IS NULL
             AND TRY_CAST(PARSE_JSON(PAYLOAD):connected::VARCHAR AS BOOLEAN) = TRUE THEN 'NO_DATA'
        ELSE 'UNPLANNED_STOP'
    END,
    TRY_CAST(REGEXP_REPLACE(PARSE_JSON(PAYLOAD):source_period::VARCHAR, '[^0-9]', '') AS NUMBER),
    TRY_CAST(PARSE_JSON(PAYLOAD):connected::VARCHAR AS BOOLEAN),
    EVENT_TS
...
BEGIN
    ...
    USING (
        SELECT
            ...
            WORK_CENTER,
            CASE
                WHEN HOUR(EVENT_TS_LOCAL)*60+MINUTE(EVENT_TS_LOCAL) BETWEEN 45 AND 405 THEN 'Morning'
                WHEN HOUR(EVENT_TS_LOCAL)*60+MINUTE(EVENT_TS_LOCAL) BETWEEN 405 AND 885 THEN 'Day'
                WHEN HOUR(EVENT_TS_LOCAL)*60+MINUTE(EVENT_TS_LOCAL) BETWEEN 885 AND 1365 THEN 'Evening'
                ELSE 'Night'
            END AS shift_name,
            ...
            SUM(CASE WHEN DOWNTIME_CATEGORY='NO_DATA' THEN SOURCE_PERIOD_SEC ELSE 0 END) AS no_order_seconds,
            0 AS material_shortage_seconds,
            SUM(CASE WHEN DOWNTIME_CATEGORY='UNPLANNED_STOP' THEN SOURCE_PERIOD_SEC ELSE 0 END) AS mechanical_fault_seconds,
            SUM(CASE WHEN DOWNTIME_CATEGORY='PLANNED_STOP' THEN SOURCE_PERIOD_SEC ELSE 0 END) AS planned_stop_seconds,
            0 AS changeover_seconds, 0 AS scheduled_maint_seconds,
